const BannerSecond = require('../models/bannersecondModel.js')
...
const createBanner = asyncHandler(async (req, res) => {
    ...
    const newBanner = await BannerSecond.create({
        title,
        images,
        type
    })
...
const updateBanner = asyncHandler(async (req, res) => {
    ...
    const banner = await BannerSecond.findById(id)
...
const deleteBanner = asyncHandler(async (req, res) => {
    const { id } = req.query
    const banner = await BannerSecond.findOneAndDelete({ _id: id })
    if (!banner) {
        return res.status(400).send({ message: 'Banner not found' })
    }
...
const deleteBannerImage = async (req, res) => {
    ...
    try {
       ...
       const banner = await BannerSecond.findByIdAndDelete(bannerId)
